...
class Simulator:
    ...
    def _save_plots(self) -> None:
        ...
        print(self.plot_joint_position.shape)
        plt.figure(figsize=(10, 6))
        for i in range(self.plot_joint_position.shape[1]):
            plt.plot(self.times, self.plot_joint_position[:, i], label=f'Joint {i+1}')
        plt.xlabel('Time [s]')
        plt.ylabel('Joint Positions [rad]')
        plt.title('Joint Positions over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(self.plots_path + '03_task_space_joint_positions.png')
        plt.close()

        print(self.plot_joint_velocity.shape)
        plt.figure(figsize=(10, 6))
        for i in range(self.plot_joint_velocity.shape[1]):
            plt.plot(self.times, self.plot_joint_velocity[:, i], label=f'Joint {i+1}')
        plt.xlabel('Time [s]')
        plt.ylabel('Joint Velocities [rad/s]')
        plt.title('Joint Velocities over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(self.plots_path + '03_task_space_joint_velocities.png')
        plt.close()


        plt.figure(figsize=(10, 6))
        plt.plot(self.times, self.plot_des_position[:, 0], label=f'x_des', linestyle='dashed', color='blue')
        plt.plot(self.times, self.plot_des_position[:, 1], label=f'y_des', linestyle='dashed', color ='green')
        plt.plot(self.times, self.plot_des_position[:, 2], label=f'z_des', linestyle='dashed', color='red')
        plt.plot(self.times, self.plot_position[:, 0], label=f'x', color='blue')
        plt.plot(self.times, self.plot_position[:, 1], label=f'y', color ='green')
        plt.plot(self.times, self.plot_position[:, 2], label=f'z', color='red')
        plt.xlabel('Time [s]')
        plt.ylabel('EE position [m]')
        plt.title('EE Position over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(self.plots_path + '03_task_space_position.png')
        plt.close()

        plt.figure(figsize=(10, 6))
        plt.plot(self.times, self.plot_des_rotation[:, 1], label=f'x_des', linestyle='dashed', color='blue')
        plt.plot(self.times, self.plot_des_rotation[:, 2], label=f'y_des', linestyle='dashed', color ='green')
        plt.plot(self.times, self.plot_des_rotation[:, 3], label=f'z_des', linestyle='dashed', color='red')
        plt.plot(self.times, self.plot_rotation[:, 1], label=f'x', color='blue')
        plt.plot(self.times, self.plot_rotation[:, 2], label=f'y', color ='green')
        plt.plot(self.times, self.plot_rotation[:, 3], label=f'z', color='red')
        plt.xlabel('Time [s]')
        plt.ylabel('EE rotation [m]')
        plt.title('EE rotation over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(self.plots_path + '03_task_space_rotation.png')
        plt.close()
    ...
    def run(self, time_limit: Optional[float] = None) -> None:
        ...
        try:
            ...
            self.plot_joint_position = np.empty((0, state['q'].size))
            self.plot_joint_velocity = np.empty((0, state['dq'].size))
            self.plot_rotation = np.empty((0, state['ee_rot'].size))
            self.plot_des_position = np.empty((0, state['desired']['pos'].size))
            self.plot_des_rotation = np.empty((0, state['desired']['quat'].size))
            self.plot_position = np.empty((0, state['ee_pos'].size))
            while (not viewer or viewer.is_running()):
                ...
                self.plot_joint_position = np.vstack([self.plot_joint_position, state['q']])
                self.plot_joint_velocity = np.vstack([self.plot_joint_velocity, state['dq']])
                self.plot_rotation = np.vstack([self.plot_rotation, state['ee_rot']])
                self.plot_des_position = np.vstack([self.plot_des_position,state['desired']['pos']])
                self.plot_position = np.vstack([self.plot_position,state['ee_pos']])
                self.plot_des_rotation = np.vstack([self.plot_des_rotation,state['desired']['quat']])
